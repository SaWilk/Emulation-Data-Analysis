...
% set input path
datapath = 'R:\AG-Beste-Studien\Emulation\06_analysis\Emulation-Data-Output';
cd(datapath);
...
chan = 'Cz';
 channr = find(strcmp(chan,{ERSP_A.chanlocs{1}.labels}));
  figure; 
   subplot(1,3,1);
   imagesc(ERSP_A.times{1},ERSP_A.freqs{1},squeeze(ga_ersps_random(:,:,channr))); ...
       title([chan '-random']);set(gca,'YDir','normal')
   subplot(1,3,2);
   imagesc(ERSP_A.times{1},ERSP_A.freqs{1},squeeze(ga_ersps_random2(:,:,channr))); ...
       title([chan '-random2']);set(gca,'YDir','normal')
   subplot(1,3,3);
   imagesc(ERSP_A.times{1},ERSP_A.freqs{1},squeeze(ga_ersps_constant(:,:,channr))); ...
       title([chan '-constant']);set(gca,'YDir','normal')
