...
for ind = 1:34
    ...
    filename = TMPEEG.filename(1:end-4);
     ...
     TMPEEG = pop_epoch( TMPEEG, {  'S 23' 'S 24' 'S 27'  }, [-1  4], 'newname', [TMPEEG.setname '-eps'], 'epochinfo', 'yes');
     TMPEEG = pop_rmbase( TMPEEG, [-1000 0] ,[]);
     TMPEEG = pop_saveset(TMPEEG,'filename',[TMPEEG.filename(1:end-4) '-eps'], 'filepath',savepath);
    ...
    TMPEEG.setname = [filename '-eps-icaclean'];
    ...
    pause;
    if isempty(ics)
        TMPEEG = pop_saveset(TMPEEG,'filepath',TMPEEG.filepath,'filename',TMPEEG.setname,'filepath',savepath);
    else
        TMPEEG = pop_subcomp( TMPEEG, ics, 0);
        TMPEEG.setname = [TMPEEG.filename(1:end-4) '-icaclean'];
        TMPEEG = pop_saveset(TMPEEG,'filepath',TMPEEG.filepath,'filename',TMPEEG.setname ,'filepath',savepath);
    end
...
% 
     %TMPEEG = pop_saveset(TMPEEG,'filepath',TMPEEG.filepath,'filename',[TMPEEG.setname '-CSD'],'filepath',savepath);
% 
%     channum = find(strcmp(chan,{TMPEEG.chanlocs.labels}));
%     traj = []; 
