...
for s = 1:length(subj_ids)
    for task = 1:2
        for t = 1:size(track_data(s).trials.(task_names{task}), 2)
            ...
            % get index of the current trial in structure
            which_error = [s, task, t];
            % save the maximum errors and mean errors with the index info in a cell
            error_max_cell{count} = {cur_max_errors, which_error};
            error_cell{count} = {cur_mean_errors, which_error};
            % get trials where trial started with dislocated joystick
            if cur_error(1) > 0.2;
...
% result of visual analysis (done in plots_and_problems)
% % trials to exclude:
% % subject, task, trial
% [24, 2, 46];
% [16, 2, 70];
...
for s = unique(sort(rem_trials(:,1)))' % took me half an hour, but for loops loop through the columns... duh!
    for task = unique(sort(rem_trials(s == rem_trials(:,1), 2)))'
        for t = rem_trials(s == rem_trials(:,1) & task == rem_trials(:,2), 3)'
            ...
            % load eeg events from structure
            ...
            % replace the eeg events in rem_trials with 'DEL' resp.
            % 'bad_trial'
            for d = 1:size({cur_trial.type},2)
                cur_trial(d).type = 'DEL';
                cur_trial(d).code = 'bad_trial';
            end
            % put eeg events back in the structure
...
% save tracking data in .mat file
