...
% check sope of error distribiton
...
%% Test the epoched data output - is it still aligned with the trajectory?

file_path = matlab.desktop.editor.getActiveFilename;
file_path = fileparts(file_path);
filepath_parts = strsplit(file_path, filesep);
grandparent_dir = filepath_parts(1:end-2);
grandparent_dir = strjoin(grandparent_dir, filesep);
track_data_path = strjoin([grandparent_dir, "Emulation-Data-Output", "03_parallelize_with_traj"], filesep);
cd(track_data_path);
