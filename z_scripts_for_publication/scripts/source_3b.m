...
% for experiment 2, 1 contrast & TBA
...
% read source data
sourcedata = load([inputpath filesep 'source_ratio_B_all_avg']);
sourcedata = sourcedata.source_ratio_all_avg;


for frq = 1:size(freq_labels, 2)
    for contr = 1:size(identifier{frq}, 1)
        data = sourcedata.(freq_labels{frq}).(identifier{frq}{contr});
        ...
        % file naming
        filename    = [freq_labels{frq} '_' identifier{frq}{contr, 1} '_' dirtext];
