function [plot_of_trigger_and_traj] = plot_trigger_on_traj(EEG_sets, traj_struct, subject, trial, task, epoch, epoch_lim)
...
% plots one epoch - requires epoched data
...
task = 'task_a';
plot_purs = true;
epoch_lim = [-.75, 0.5]; % epoch limits in ms
...
% if there is only one dataset in the data
...
% if epoch limits are given in seconds, adjust to milliseconds
if epoch_lim(1) < 10
    epoch_lim = epoch_lim * 1000;
end
