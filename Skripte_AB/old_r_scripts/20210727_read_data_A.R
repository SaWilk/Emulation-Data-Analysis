...
subj_ids = matrix(NA, length(files_A), 2) 
for (i in 1:n_files_A) {
  subj_ids[i ,1] = substr(files_A[i], 10, 14)
  subj_ids[i, 2] = i
}
...
for (i in 1:n_files_A) {
  filename = paste("Tracking-", subj_ids[i, 1], "_A-Trials.npz", sep = "")
...
for (i in 1:n_files_A) {
  median_split = vector()
  median_rmsd = median(list_a[[i]]$rmsd)
  for (j in 1:trialnum) {
    median_split[j] = ifelse(list_a[[i]]$rmsd[j] >= median_rmsd, 1, 0)
    list_a[[i]]$median_split = median_split
  }
} 
...
for (i in 1:n_files_A) {
  folder_name = paste(subj_ids[i], "_A", sep = "")
  file_name = paste(subj_ids[i], "_A_trialdata.csv", sep = "")
